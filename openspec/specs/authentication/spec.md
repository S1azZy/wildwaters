...
- **THEN** it shows an account dropdown trigger instead of the guest sign-in action
- **AND** the dropdown contains a disabled Profile item
- **AND** the dropdown contains a bottom Log out action that submits the existing Rails-owned `DELETE /session` request
...
- **AND** it does not display the account dropdown, Profile item, Admin item, or Log out action
...
- **WHEN** an application-owned Inertia page renders
- **THEN** the shared account dropdown contains an Admin action targeting the Rails-generated admin URL
