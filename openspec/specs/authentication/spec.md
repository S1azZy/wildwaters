...
- **THEN** the system creates a session and redirects to the explore homepage

#### Scenario: Invalid credentials
- **GIVEN** an unknown email or an incorrect password
- **WHEN** a visitor attempts to sign in
- **THEN** the system creates no session
- **AND** it returns an unprocessable response with the same generic failure message
...
The system SHALL require an authenticated session for the dashboard.

#### Scenario: Guest opens dashboard
- **GIVEN** a visitor without an active session
- **WHEN** the visitor opens the dashboard
- **THEN** the system redirects to sign-in with an authentication-required message
...
- **THEN** the system records the session as revoked
- **AND** it clears the session cookie and redirects to sign-in
